...
def pytest_addoption(parser):
    ...
    """
    # parser.addoption("--target-browser", default="chromium", help="Browser: chromium, firefox, webkit")
    # parser.addoption("--headed_mode", action="store_true", help="Run in headed (visible) mode")
    # parser.addoption("--base-url", default="https://tutorialsninja.com/demo/", help="Base URL for tests")
    # parser.addoption("--record_video", default="retain-on-failure", help="Record video: on, off, retain-on-failure")
...
# USING:
#   Pytest hooks that pass "config" as a direct parameter
#   (e.g., pytest_configure(config))
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
@pytest.hookimpl(hookwrapper=True) #Pre-defined HOOK name to capture test results
def pytest_runtest_makereport(item, call): # Hook function with fixed name
    """
...
def browser_context(request):
    ...
    # Yield(return) the context for use in tests
    yield context
...
def page(request, browser_context):
    ...
    yield page # all below actions can be done only after execution is complete, i.e after yield
